...
public class CategoryController {
	...
	public ResponseEntity<?> addCategory(@RequestBody CategoriesDialQuizz category){
		CategoriesDialQuizz theCategory=this.categoryService.addCategory(category);
		return ResponseEntity.ok(theCategory);
	}
	
	@GetMapping("/{categoryId}")
	public ResponseEntity<CategoriesDialQuizz> getCategory(@PathVariable("categoryId") Long cid) throws Exception {
		System.out.println("Category id requested from the user is "+cid);
		CategoriesDialQuizz category=this.categoryService.getCategoryById(cid);
		if(category==null) {
	...
	public ResponseEntity<?> getAllCategory() throws Exception{
		Set<CategoriesDialQuizz> categories=this.categoryService.getCategories();
		if(categories==null) {
	...
	public ResponseEntity<?> updateCategory(@RequestBody CategoriesDialQuizz category) {
		CategoriesDialQuizz thecategory=this.categoryService.updateCategory(category);
		return ResponseEntity.ok(thecategory);
	}
